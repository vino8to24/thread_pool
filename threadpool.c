...
threadpool_t *threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size) {
	...
	do{
		...
		{
			printf("initialize the lock or cond failed.");
			break;
		}
...
void *threadpool_thread(void *threadpool){
	...
	while(true){
		...
		// queue_size == 0说明没有任务，调用wait阻塞条件变量，有任务跳过该while
		...
		pool->queue_front = (pool->queue_front+1) % pool->queue_max_size; //模拟环形队列
		pool->queue_size--;
		...
		// 任务结束
		printf("thread 0x%x finished work...\n", (unsigned int)pthread_self());
...
void *adjust_thread(void *threadpool){
	...
	while(!pool->shutdown){
		...
		if((busy_thr_num *2) < live_thr_num && live_thr_num > pool->min_thr_num){
			// 一次销毁 DEFAULT_THREAD 个线程，随机10个即可
			pthread_mutex_lock(&(pool->lock));
